...
class RecurlySettingsForm extends RecurlyConfigForm {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($entity_types as $entity_name => $bundles) {
      ...
      $entity_options[$entity_name] = $entity_type->getSingularLabel();
      ...
      if (count($bundles) > 1 || $first_bundle_name !== $entity_name) {
        foreach ($bundles as $bundle_name => $bundle_info) {
          // It's not a requirement for a bundle type to have a label, and the
          // Group module which is commonly used with this module, uses NULL
          // for the labels on its group_content_type entities. However, that
          // causes issues here because you end up with invalid form options
          // that result in an 'An illegal choice has been detected.' error
          // when validating the form. Making this form impossible to submit if
          // the Group module is enabled. Maybe better to fix this in the group
          // module but in the meantime we can use the bundle name if the label
          // is NULL.
          // See https://www.drupal.org/project/group/issues/3324506
          $entity_type_options[$entity_name][$bundle_name] = $bundle_info['label'] ?? $bundle_name;
        }
      }
    }
    ...
    $pages_previous_values = [
      ...
      'recurly_quantity_page' => $this->config('recurly.settings')->get('recurly_quantity_page') ?: 1,
    ...
    $form['pages']['recurly_subscription_multiple'] = [
      '#title' => $this->t('Enable multiple subscriptions to the same plan'),
      '#type' => 'checkbox',
      '#default_value' => $this->config('recurly.settings')->get('recurly_subscription_multiple'),
      '#description' => $this->t('All users to subscribe to the same plan multiple times and display options for changing the quantity in the UI.'),
      '#states' => $pages_enabled,
    ];
    ...
    $form['pages']['recurly_subscription_max'] = [
      ...
      '#description' => $this->t('Single-plan mode allows users only one subscription at a time, preventing them from having multiple plans active simultaneously. Use Multiple-plan mode if each user is allowed to sign up for more than one subscription at the same time.'),
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue(['recurly_private_api_key'])) {
      try {
        ...
        // Initialize a client with the provided settings and see if we can make
        // a successful request.
        $client = $this->recurlyClient->getClientFromSettings($settings);
        \Recurly_PlanList::get(NULL, $client);
      }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $this->config('recurly.settings')
      ...
      ->set('recurly_subscription_multiple', $form_state->getValue('recurly_subscription_multiple'))
